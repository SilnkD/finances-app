...
export class GoalsController {
    ...
    @ApiResponse({ status: 200, description: 'Цели успешно получены', type: [GetGoalDto] })
    @ApiResponse({ status: 401, description: 'Пользователь не авторизован' })
    ...
    @ApiResponse({ status: 200, description: 'Цели пользователя успешно получены', type: [GetGoalDto] })
    @ApiResponse({ status: 401, description: 'Пользователь не авторизован' })
    @UseGuards(IdGuard)
    @ApiBearerAuth()
    @Get('user')
    async getUserGoals(@Request() req): Promise<GetGoalDto[]> {
    ...
    @ApiResponse({ status: 200, description: 'Цель успешно обновлена', type: GetGoalDto })
    @ApiResponse({ status: 401, description: 'Пользователь не авторизован' })
    ...
    @ApiBearerAuth()
    @Put('/:id')
    async updateGoal(@Body() updateGoalDto: CreateGoalDto, @Request() req, @Param('id') id: number): Promise<GetGoalDto | HttpException> {
        const userId = req.user.id;
